...
//==================================================================================
 ...
 router.post('/subject/uploadvideo', uploadvideo);      //上传学习情景中节或小节的视频
 router.get('/subject/downloadvideo', downloadvideo); //上传学习情景中节或小节的视频
 
 router.post('/subject/searchsections', searchsections); //查询章下面的节
 router.post('/subject/sectionoperation', sectionoperation); //节的操作
 router.post('/subject/sectionchildoperation', sectionchildoperation); //小节的操作
 router.post('/subject/commentsoperation', commentsoperation); //公共评论的操作
 router.post('/subject/studylogoperation', studylogoperation); //公共评论的操作
...
function uploadpngchild(req,res,next) {
   ...
   console.log(pngpath);
    fs.exists(pngpath,function(flag){
        if (flag == false) return res.send({ code: 204, err:"视频文件夹不存在"}); //文件夹不存在
...
function uploadvideo(req, res, next) {
    ...
    var vediopath = config[mgenv].courseDeve.videopath;
    console.log(vediopath);
    fs.exists(vediopath, function (flag) {
        if (flag == false) return res.send({ code: 204, err: "视频文件夹不存在" }); //文件夹不存在

        uploadFile(vediopath, req, function (err, fileDoc) {
            if (err) return res.send({ code: 204, err: err });
            res.send({ code: 201, datas: { path: fileDoc.path, vtype: fileDoc.filetype, videoname: fileDoc.name } });
        }); // fs.exists end
    }); //uploadFile 
...
//查询章下节的集合数据
function searchsections(req, res, next) {
    var result = [], course_child_id = req.body.course_child_id;
    if (course_child_id == '' || course_child_id == undefined) return res.send({ code: 204, err: 'course_child_id参数不正确' });

    console.log('course_child_id:', course_child_id);
    var sqlstr = "CALL stroed_searchsections('" + course_child_id + "')";
    mysql.query(sqlstr, function (err, tmpdocs) {
        if (err) return res.send({ code: 204, err: err });
        
        var result = tmpdocs[0];
        console.log('11111:', result);

        if (result.length <= 0) { result.childs = []; return res.send({ code: 201, datas: result }); }

        var flag = 0, returnFlag = 0;
        for (var i = 0; i < result.length; i++) {
            (function (i) {
                var sqlstr_2 = " CALL  stroed_searchsectionchilds('" + result[i].section_id + "')  ";
                console.log(i, sqlstr_2);
                mysql.query(sqlstr_2, function (err, tmpdocs_2) {
                    if (err) { console.log(err); if (returnFlag == 0) { returnFlag = 1; return res.send({ code: 204, err: err }); } else return; }
                    console.log(tmpdocs_2[0]);
                    result[i].childs = tmpdocs_2[0];
                    flag++;
                    if (flag == result.length) { return res.send({ code: 201, datas: result }); returnFlag = 1; }
                });
            })(i);
            }
...
//节的操作
function sectionoperation(req, res, next) {
    var type = req.body.type;
    ...
    if (type == 1) { //新建
        ...
        if (req.body.video != '' && req.body.video != undefined) data.video = req.body.video;
        if (req.body.videoname != '' && req.body.videoname != undefined) data.videoname = req.body.videoname;
        if (req.body.vtype != '' && req.body.vtype != undefined) data.vtype = req.body.vtype;
        if (req.body.title != '' && req.body.title != undefined) data.title = req.body.title;

   
        var table = config[mgenv].mysql.header + "_cd_section";
        templater.add(table, data, function (err, doc) {
            if (err) return res.send({ code: 204, err: err });
            else return res.send({ code: 201, datas: { section_id: data.section_id } });
        }); //templater.add  end	  
    ...
    if (type == 2) { // 更改
        ...
        if (req.body.videoname != undefined) data.videoname = req.body.videoname;
        if (req.body.vtype != undefined) data.vtype = req.body.vtype;
        if ( req.body.title != undefined) data.title = req.body.title;
    ...
    if (type == 4) { //查询
        ...
        mysql.query("CALL stroed_get_section( '" + section_id + "' )", function (err, tmpdocs) {
            if (err) return res.send({ code: 204, err: err });
            var docs = tmpdocs[0];
            return res.send({ code: 201, datas: docs[0] });
        });
...
//小节的操作
function sectionchildoperation(req, res, next) {
    var type = req.body.type;
    if (type != 1 && type != 2 && type != 3 && type != 4) return res.send({ code: 204, err: 'type参数不正确' });
    ...
    if (type == 1) { //新建
        ...
        templater.add(table, data, function (err, doc) {
            if (err) return res.send({ code: 204, err: err });
            else return res.send({ code: 201, datas: { section_child_id: data.section_child_id } });
        }); //templater.add  end	  
    ...
    if (type == 4) { //查询
        ...
        mysql.query("CALL stroed_get_section_child( '" + section_child_id + "' )", function (err, tmpdocs) {
            if (err) return res.send({ code: 204, err: err });
            var docs = tmpdocs[0];
            return res.send({ code: 201, datas: docs[0] });
        });
...
//公共评论的操作
function commentsoperation(req, res, next) {
    var type = req.body.type, section_child_id = req.body.section_child_id, section_id = req.body.section_id ;
    if (type != 1 && type != 2 && type != 3) return res.send({ code: 204, err: 'type参数不正确' });
    ...
    if (type == 1) { //新建
        ...
        data.user_id = req.session.userinfo.user_id, data.account = req.session.userinfo.account;
        data.headpngpath = req.session.userinfo.headpngpath ,  data.fullname = req.session.userinfo.fullname;

        var table = config[mgenv].mysql.header + "_cd_public_comments";
        templater.add(table, data, function (err, doc) {
            if (err) return res.send({ code: 204, err: err });
            else return res.send({ code: 201, datas: { public_comments_id: data.public_comments_id } });
        }); //templater.add  end	  
    ...
    if (type == 3) { //查询
        if (req.body.section_id == undefined && req.body.section_child_id == undefined) return res.send({ code: 204, err: '外键参数不正确' });

        if (req.body.section_id != undefined) { var ID = req.body.section_id, typetmp = '1'; }
        if (req.body.section_child_id != undefined) { var ID = req.body.section_child_id, typetmp = '2'; }

        mysql.query("CALL stroed_get_comments( '" + ID + "' , '" + typetmp + "' )", function (err, tmpdocs) {
            if (err) return res.send({ code: 204, err: err });
            var docs = tmpdocs[0];
...
//学习记录的操作
function studylogoperation(req, res, next) {
    var type = req.body.type;
    if (type != 1 && type != 2 && type != 3) return res.send({ code: 204, err: 'type参数不正确' });
    ...
    if (type == 1) { //新建
        ...
        templater.add(table, data, function (err, doc) {
            if (err) return res.send({ code: 204, err: err });
            else { //检查是否存在试题： 获取试题ID， 实例化试题  ,返回myexamid
                
                if (section_id != undefined)       var sqlstr = "CALL stroed_get_section( '" + section_id + "' )";
                if (section_child_id != undefined) var sqlstr = "CALL stroed_get_section_child( '" + section_child_id + "'  )";  

                mysql.query(sqlstr, function (err, tmpdocs) {
                    if (err) return res.send({ code: 204, err: err });

                    var exam_id = tmpdocs[0][0].exam_id, user_id = data.user_id;
                    if (exam_id == '' || exam_id == null) return res.send({ code: 201, datas: { study_log_id: data.study_log_id  } });

                    study.initExam_V3({ exam_id: exam_id, user_id: user_id }, function (err, myexamdoc) {
                        if (err) return res.send({ code: 204, err: err });

                        var table = config[mgenv].mysql.header + "_cd_study_log";
                        var wherejson = { study_log_id: data.study_log_id  };
                        templater.update(table, wherejson, { myexam_id: myexamdoc.myexam_id  }, function (err, doc) {
                            if (err) return res.send({ code: 204, err: err });
                            return res.send({ code: 201, datas: { study_log_id: data.study_log_id  }  });
                        });
                        
                    }) //study.initExam_V3 end
    ...
    if (type == 2) { //更新
        ...
        var table = config[mgenv].mysql.header + "_cd_study_log";
        var wherejson = { study_log_id: study_log_id };
        templater.update(table, wherejson, data, function (err, doc) {
            if (err) return res.send({ code: 204, err: err });
            else res.send({ code: 201 });
        });
    ...
    if (type == 3) { //查询
        ...
        if (req.body.section_child_id != undefined) { var ID = req.body.section_child_id, typetmp = '2'; }
        var user_id = req.session.userinfo.user_id;
        console.log("CALL stroed_get_studylog( '" + user_id + "'  , '" + ID + "' , '" + typetmp + "' )");
        mysql.query("CALL stroed_get_studylog( '" + user_id + "'  , '" + ID + "' , '" + typetmp + "' )", function (err, tmpdocs) {
            if (err) return res.send({ code: 204, err: err });
            var docs = tmpdocs[0];
            return res.send({ code: 201, datas: docs });
        });
