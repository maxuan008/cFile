...
(function(container){
    container.empty().append('<div class="j-title"><div><input type="text"/><span class="j-search">搜索</span></div></div><div class="j-manage-main"><div class="j-page"><div class="j-page-switch"><i class="j-icon j-page-prev">&#xe314</i><p class="j-current-page">1</p><p>/</p><p class="j-total-page">25</p><i class="j-icon j-page-next">&#xe315</i></div></div><div class="j-course-list"></div></div>');
    ...
    var setPage = function(datas){
        ...
        totalPage = Math.ceil(datas.length/5);
        ...
        for(i = 0;i < totalPage;i++){
            ...
            if(i<totalPage - 1){
                for(j=0;j<5;j++){
                    tmpArr.push(datas[i*5+j]);
                }
            }else{
                for(j=0;j<(datas.length-((totalPage-1)*5));j++){
                    tmpArr.push(datas[i*5+j]);
                }
            }
    ...
    var changePage = function(p){
        if(p === '-1'){
            ...
            $(courseData[p]).each(function(i,one){
                $('.j-course-list').append('<div data-id="'+one.course_id+'" class="j-course-one '+(i%4 === 0?'no-margin':'')+'"><img src="'+(one.pngpath?(ports.course.domain == location.host ? ports.course.domain : '')+'/course/develop/downloadpng?path='+one.pngpath:'/img/default/course.png')+'"/><h4>'+one.coursename+'</h4><span>'+one.period+'课时</span></div>');
            });
