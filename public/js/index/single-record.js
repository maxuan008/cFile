...
(function(container){
    container.empty().append('<div class="j-record-c"><div class="j-record"><p>课程</p><div style="z-index: 2"><p>-请选择-</p><i class="j-icon">&#xe313</i><span class="j-org-select" data-name="course">1234</span></div></div><div class="j-record"><p>组织机构</p><div style="z-index: 1"><p>-请选择-</p><i class="j-icon">&#xe313</i><span class="j-org-select" data-name="org">1234</span></div></div><button>搜索</button></div><div class="j-record-header"><span >课程名</span><span>用户</span><span>学习状态</span><span>最后学习时间</span><span>操作</span></div><div class="j-record-body"></div><div class="j-record-footer"><div></div></div>');
    ...
    var bindEvents = function(){
        ...
        $('.j-record-body').on('click','button.show-detail',function(){
            ...
            if(init === '0'){
                ...
                sendMessage('post',(ports.study.domain === location.host ? ports.study.domain : ''),'/study/many/gettaskhistory',{study_log_id:study_log_id,task_id:task_id},function(data){
                    if(data.code === 201){
    ...
    var createCourseMap = function(courseData){
        var addActivity = function(activity,index,total){
            var addStep = function(step,index,total,con){
                var task = step.tasks[0];
                var task_con = document.createElement('span');$(task_con).attr('data-id',task.task_id);task_con.className = 'j-icon j-task-ele';task_con.innerHTML = '&#xe5ca';
                var div = document.createElement('div');$(div).attr('data-id',task.task_id).attr('data-init','0');div.className = 'j-record-task-detail';$(task_con).append(div);
                $(con).append(task_con);
                if(index < (total-1)){
                    var director = document.createElement('i');director.className = 'j-icon j-map-director';director.innerHTML = '&#xea66';
                    $(con).append(director);
    ...
    var recordChangePage = function(index){
        if(index === 'prev'){
            ...
            $(recordArray[index]).each(function(i,r){
                $('.j-record-body').append('<div class="j-record-list" data-id="'+r.study_log_id+'"><span>'+sendData.coursename+'</span><span>'+(r.fullname?r.fullname:r.account)+'</span><span>'+(r.over_time?'已学完':'学习中')+'</span><span class="time">'+(r.over_time?new Date(r.over_time).Format('yyyy-MM-dd hh:mm:ss'):'-')+'</span><span><button class="show-detail" data-open="0">详情</button><button class="comment">评价</button></span><div class="j-record-detail" data-id="'+r.study_log_id+'"></div></div>');
            });
    ...
    var initPageSearch = function(){
        var recordSetPage = function(){
            $(userArray.users).each(function(i,r){
                var page,index;
